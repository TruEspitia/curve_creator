...
function setupListeners() {
    ...
    document.getElementById('btn-fit').addEventListener('click', async () => {
        ...
        if (currentTab === 'predefined') {
            ...
            result = await eel.run_fit(funcName, colX, colY)();
        } else {
...
async function refreshFunctions() {
    let functions = await eel.get_functions_list()();
...
function displayResults(result) {
    ...
    let html = `<div><strong>R²:</strong> ${result.r_squared.toFixed(4)}</div><hr/>`;

    for (let [key, val] of Object.entries(result.parameters)) {
        let err = result.errors[key] || 0;
        html += `<div><strong>${key}:</strong> ${val.toFixed(4)} ± ${err.toFixed(4)}</div>`;
    }
